...
import {
  ...
  type GenerateDailySummaryResult,
} from "@/lib/jobs/generateDailySummary";
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (typeof timezone !== "string") {
    return NextResponse.json(
      { ok: false, error: "Missing or invalid timezone" },
      { status: 400 },
    );
  }
  ...
  let result: GenerateDailySummaryResult;
  try {
    result = await generateDailySummary(userPb, {
      userId: user.id,
      timezone,
    });
  } catch (err) {
